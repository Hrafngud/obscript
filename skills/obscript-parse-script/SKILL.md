...
Read the approved structured script and the storybook scene plan, or apply these rules while creating that plan. Use the existing scene intervals and shared creative direction. Natural-language scene instructions are PT-BR. This is visual pre-production: return a storybook conforming to [the existing schema](../../schemas/storybook.schema.json), without invoking HyperFrames or generating media.
...
For example, a scene from 10 to 16 seconds with `Veja o *círculo* e a **seta**.` has six spoken words. The circle cue begins at approximately 12 seconds (scene-local 2 seconds), and the arrow cue at approximately 15 seconds (scene-local 5 seconds). Fit the arrow's wiggle into the remaining second. A suitable emphasis instruction is: “Na ocorrência ‘círculo’, ampliar o círculo de 1 para 1,12 e retornar a 1 entre 2,00–2,80 s locais (12,00–12,80 s absolutos, estimados). Na ocorrência ‘seta’, oscilar a seta em ±4° e restaurar a rotação entre 5,00–6,00 s locais (15,00–16,00 s absolutos, estimados).”
