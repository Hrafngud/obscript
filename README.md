...
obscript PROJECT_ID --post-production --post-production-batch-size 10
...
Without a phase flag, a run stops after script review. `--storybook` continues through visual planning and validation, producing the timestamp-free `script-readable.md`, the timed `script.md`, an Obsidian-readable `storybook.md`, and the structured `storybook.yaml`, without rendering. `--render` runs any missing script and storybook phases, then creates silent animations through the installed `$hyperframes` skill. These flags are mutually exclusive. Rendering processes at most 20 pending scenes per model iteration by default; `--render-batch-size SCENES` sets another positive limit. The last iteration always processes the remaining scenes and performs final assembly.
...
The approved structured script in `.obscript/approved-script.json` is the sole spoken source. Scenes partition its narration into exact contiguous excerpts, each bound to one section. The shared creative-direction Markdown supplies visual standards to all videos. `storybook.md` links to that source, and `.obscript/creative-direction-source.json` records its absolute path and content hash without copying its standards. `storybook.md` displays an English timeline and a concise Scene / Layout paragraph for each scene: concrete elements, asset references, positions, backgrounds, timed animation, camera moves, and transitions. These production directions describe what appears rather than re-explaining narration. Narration and on-screen labels keep their original language. Storybook schema 3 requires every scene to resolve five design pillars together: context accuracy, recorded local-asset search and selection, explanatory animation beyond entrances and exits, a semantically mapped visual abstraction, and one mute-readable attention anchor. Validation requires context-relevant foreground library assets in at least `ceil(4n/5)` scenes and verified raster backgrounds from `background1` in at least `ceil(n/5)` scenes; selected paths must be traceable through the scene's structured production fields. An explicit `--storybook` run rebuilds an obsolete storybook contract, while `--render` blocks until that rebuild occurs. `storybook.yaml` retains the complete structured scene fields and exact narration references. `storybook.yaml` remains the structured production plan and is revalidated before rendering. Scene durations primarily fall between 3 and 12 seconds and become the animation's allocated intervals. `voiceover.text` remains the exact human narration reference, not a request to generate speech. The same boundaries appear in `script.md` as `HH:MM:SS.mmm` cues at section and scene level, while `script-readable.md` preserves the approved section-based script without timestamps for comfortable reading. On-screen text supplements narration. Text density is a layout and readability choice, with no fixed word-count limit and no word-count validation gate.

Rendering uses sequential runs of the selected harness through `ProductionAgent`, which invokes `$produce-video` and delegates animation execution to the installed `$hyperframes` skill. Each handoff contains the verbatim shared creative-direction Markdown, its source path, and one bounded storyboard batch with immutable narration references, planned timestamps, and scene output destinations. All iterations resume the same editable project at `production/hyperframes/`; only the final iteration receives the compact all-scene assembly map and final video destination. The producer writes `BRIEF.md` after initialization and extends the project across batches. Defaults are 20 pending scenes per iteration, 1920×1080, and 30 fps; the explicit `--render` request supplies render authorization after required quality checks.

Scene planning and rendering prefer the local visual asset library at `/home/zalmo/documents/obsidian/Videos/Videos/Globals/assets` for general illustrations, technical icons, emojis, raster backgrounds, and textures, including animated interactions. Agents inspect existing files before selecting them and use suitable library assets before inventing SVG foreground artwork, generating images, or searching external stock. Production copies selected assets into its editable project, carries the preference and asset mapping into `BRIEF.md`, and may recolor monochrome fills/strokes or animate icons and internal parts to match the shared direction and scene timing. At least `ceil(total scenes / 5)` scenes, and at least one scene in every nonempty video, visibly use a custom raster image from `background1` as a full-frame or substantial background region rather than only a plain base. These scenes are distributed across the timeline. Background images are evaluated by intrinsic dimensions, aspect ratio, and visible content: sufficiently resolved images can cover the frame, while tiny raster textures are repeated at a suitable scale across a substantial region. Backgrounds and patterns may be rotated, tilted, cropped, or layered, with enough coverage to avoid gaps and preserve foreground readability. SVGs are never generated or used as backgrounds, including SVG files present in a background collection; SVG icons and illustrations remain available as foreground elements. The shared library remains unchanged; explicit assets and meaningful brand or emoji colors are preserved.
...
`obscript PROJECT_ID --post-production [INSTRUCTION]` runs a separate visual finishing pass on a completed render. The optional quoted instruction is addressed alongside the full standard polish pass; it does not replace the overall inspection. It requires an existing project ID and is mutually exclusive with `--storybook` and `--render`; `new` does not support it. The pass does not transcribe, rewrite, review, replan, or automatically perform the initial render. It processes at most 20 scenes per model iteration by default; `--post-production-batch-size SCENES` sets another positive limit, and the final partial batch is always processed. Every video in a split or playlist project must have a completed render, and all are checked before polishing begins. Stale approved inputs, changed shared standards or storybooks, missing editable source, changed original video, invalid scene media, or missing ffprobe block the pass. Rebuild changed standards with `--storybook`, then run `--render` before retrying.

The dedicated `$post-production` agent inspects the existing video and edits a copy of `production/hyperframes/` under `post-production/hyperframes/`. Sequential batches resume that same copy and concentrate on flat or poorly polished moments: compatible vignettes and overlay textures, clearer element-focused effects, and varied transitions motivated by adjacent scenes. Each batch retains its own report fragment; after the final batch renders and verifies the complete video, the application combines them into `post-production/report.md`. The pass preserves narration, meaning, shared visual identity, scene order, and every planned boundary.
...
Single-batch request and prompt files use the original `post-production` names. Multi-batch runs retain numbered requests and prompts in `.obscript/`, plus numbered responses, executor logs, and report fragments under `post-production/` for inspection.
